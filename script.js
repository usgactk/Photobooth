...
galleryInput.addEventListener('change', async (e) => {
    ...
    // Seçilen dosyaları mevcut listeye ekle (Maksimum 4 olana kadar)
    ...
    els.counter.innerText = state.count; // Varsa sayaç güncellensin

    // Eğer henüz 4 fotoğraf dolmadıysa kullanıcıya uyarı ver ve seçiciyi temizle
    if (state.count < state.max) {
        alert(`Şu an ${state.count} fotoğraf seçildi. Lütfen ${state.max - state.count} tane daha seçin.`);
        galleryInput.value = ""; // Aynı butona tekrar basabilmesi için temizle
    } else {
        // 4 fotoğraf tamamlandığında editöre geç
